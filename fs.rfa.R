...
#' The web page \url{http://topepo.github.io/caret/recursive-feature-elimination.html#rfa}
#' has more details and examples related to this function.
#'
#' \code{rfa} can be used with "explicit parallelism", where different
#' resamples (e.g. cross-validation group) can be split up and run on multiple
#' machines or processors. By default, \code{rfa} will use a single processor
...
#' in parallel, the code for \code{rfa} does not change; prior to the call to
#' \code{rfa}, a parallel backend is registered with \pkg{foreach} (see the
#' examples below).
#'
#' \code{rfaIter} is the basic algorithm while \code{rfa} wraps these
#' operations inside of resampling. To avoid selection bias, it is better to
#' use the function \code{rfa} than \code{rfaIter}.
#'
#' When updating a model, if the entire set of resamples were not saved using
#' \code{rfaControl(returnResamp = "final")}, the existing resamples are
#' removed with a warning.
#'
#' @aliases rfa rfa.default rfaIter predict.rfa update.rfa
...
#' \code{\link{rfaControl}}, the value of \code{metric} should match one of the
#' arguments.
#' @param maximize a logical: should the metric be maximized or minimized?
#' @param rfaControl a list of options, including functions for fitting and
#' prediction. The web page
#' \url{http://topepo.github.io/caret/recursive-feature-elimination.html#rfa} has more
#' details and examples related to this function.
#' @param object an object of class \code{rfa}
...
#'
#' set.seed(1)
#' lmProfile <- rfa(x, logBBB,
#'                  sizes = c(2:25, 30, 35, 40, 45, 50, 55, 60, 65),
#'                  rfaControl = rfaControl(functions = lmFuncs,
#'                                          number = 200))
#' set.seed(1)
#' lmProfile2 <- rfa(x, logBBB,
#'                  sizes = c(2:25, 30, 35, 40, 45, 50, 55, 60, 65),
#'                  rfaControl = rfaControl(functions = lmFuncs,
#'                                          rerank = TRUE,
#'                                          number = 200))
...
#'        auto.key = TRUE)
#'
#' rfProfile <- rfa(x, logBBB,
#'                  sizes = c(2, 5, 10, 20),
#'                  rfaControl = rfaControl(functions = rfFuncs))
#'
#' bagProfile <- rfa(x, logBBB,
#'                   sizes = c(2, 5, 10, 20),
#'                   rfaControl = rfaControl(functions = treebagFuncs))
#'
#' set.seed(1)
#' svmProfile <- rfa(x, logBBB,
#'                   sizes = c(2, 5, 10, 20),
#'                   rfaControl = rfaControl(functions = caretFuncs,
#'                                           number = 200),
...
#'
#' set.seed(2)
#' ldaProfile <- rfa(train, trainClass,
#'                   sizes = c(1:10, 15, 30),
#'                   rfaControl = rfaControl(functions = ldaFuncs, method = "cv"))
...
"rfa.default" <-
           ...
           rfaControl = rfaControl(), ...)
  {
    ...
    if(is.null(rfaControl$index)) 
      rfaControl$index <- switch(tolower(rfaControl$method),
                                 cv = createFolds(y, rfaControl$number, returnTrain = TRUE),
                                 repeatedcv = createMultiFolds(y, rfaControl$number, rfaControl$repeats),
                                 loocv = createFolds(y, length(y), returnTrain = TRUE),
                                 boot =, boot632 = createResample(y, rfaControl$number),
                                 test = createDataPartition(y, 1, rfaControl$p),
                                 lgocv = createDataPartition(y, rfaControl$number, rfaControl$p))

    if(is.null(names(rfaControl$index))) names(rfaControl$index) <- prettySeq(rfaControl$index)
    if(is.null(rfaControl$indexOut)){
      rfaControl$indexOut <- lapply(rfaControl$index,
                                    function(training, allSamples) allSamples[-unique(training)],
                                    allSamples = seq(along = y))
      names(rfaControl$indexOut) <- prettySeq(rfaControl$indexOut)
    }
    ...
    test <- rfaControl$functions$summary(testOutput, lev = classLevels)
    ...
    if(is.null(rfaControl$seeds))
    {
      seeds <- vector(mode = "list", length = length(rfaControl$index))
      seeds <- lapply(seeds, function(x) sample.int(n = 1000000, size = totalSize))
      seeds[[length(rfaControl$index) + 1]] <- sample.int(n = 1000000, size = 1)
      rfaControl$seeds <- seeds
    } else {
      if(!(length(rfaControl$seeds) == 1 && is.na(rfaControl$seeds)))
      {
        ## check versus number of tasks
        numSeeds <- unlist(lapply(rfaControl$seeds, length))
        badSeed <- (length(rfaControl$seeds) < length(rfaControl$index) + 1) ||
          (any(numSeeds[-length(numSeeds)] < totalSize))
        if(badSeed) stop(paste("Bad seeds: the seed object should be a list of length",
                               length(rfaControl$index) + 1, "with",
                               length(rfaControl$index), "integer vectors of size",
                               totalSize, "and the last list element having a",
                               "single integer"))
      }
    }

    if(rfaControl$method == "LOOCV")
    {
      tmp <- looRfeWorkflow(x, y, sizes, ppOpts = NULL, ctrl = rfaControl, lev = classLevels, ...)
      ...
      tmp <- nominalRfeWorkflow(x, y, sizes, ppOpts = NULL, ctrl = rfaControl, lev = classLevels, ...)
    ...
    bestSubset <- rfaControl$functions$selectSize(x = externPerf,
                                                  metric = metric,
                                                  maximize = maximize)

    bestVar <- rfaControl$functions$selectVar(selectedVars, bestSubset)

    finalTime <- system.time(
      fit <- rfaControl$functions$fit(x[, bestVar, drop = FALSE],
                                      y,
                                      first = FALSE,
                                      last = TRUE,
                                      ...))
    ...
    if(!(rfaControl$method %in% c("LOOCV"))) {
      resamples <- switch(rfaControl$returnResamp,
                          none = NULL,
                          all = resamples,
    ...
    out <- structure(
      list(
        pred = if(rfaControl$saveDetails) do.call("rbind", tmp$everything[names(tmp$everything) == "predictions"]) else NULL,
        ...
        dots = list(...)),
      class = "rfa")
    if(rfaControl$timingSamps > 0)
    {
      out$times$prediction <- system.time(predict(out, x[1:min(nrow(x), rfaControl$timingSamps),,drop = FALSE]))
...
#' @export
rfa.formula <- function (form, data, ..., subset, na.action, contrasts = NULL)
{
...
#' @method print rfa
#' @export
print.rfa <- function(x, top = 5, digits = max(3, getOption("digits") - 3), ...)
{
                    ...
                    ...)
{
  ...
  for(k in seq(along = sizeValues))
  {
    ...
    flush.console()
    fitObject <- rfaControl$functions$fit(x[,retained,drop = FALSE], y,
    ...
    modelPred <- rfaControl$functions$pred(fitObject, testX[,retained,drop = FALSE])
    ...
    rfaPred <- if(k == 1) modelPred else rbind(rfaPred, modelPred)
    ...
    if(!exists("modImp")) ##todo: get away from this since it finds object in other spaces
    {
      ...
      modImp <- rfaControl$functions$rank(fitObject, x[,retained,drop = FALSE], y)
      if(rfaControl$verbose)
      {
        cat("-(rfa) imp",
            ifelse(label != "",
    ...
    if(nrow(modImp) < sizeValues[k]) {
      msg1 <- paste0("rfa is expecting ", sizeValues[k],
  ...
  list(finalVariables = finalVariables, pred = rfaPred)
...
#'
#' @aliases plot.rfa ggplot.rfa
#' @param x an object of class \code{\link{rfa}}.
...
#' defaults.
#' @param data an object of class \code{\link{rfa}}.
...
#' @author Max Kuhn
#' @seealso \code{\link{rfa}}, \code{\link[lattice]{xyplot}},
...
#'
#' More details on this function can be found at
#' \url{http://topepo.github.io/caret/recursive-feature-elimination.html#rfa}.
#'
...
#' in the call to \code{rfa} } The function should return a model object that
...
#' @return A list
#' @author Max Kuhn
#' @seealso \code{\link{rfa}}, \code{\link{lmFuncs}}, \code{\link{rfFuncs}},
...
#'
#' @export rfaControl
rfaControl <- function(functions = NULL,
                       rerank = FALSE,
                       method = "boot",
                       saveDetails = FALSE,
...
#' the algorithm via the \code{functions} argument of \code{\link{rfaControl}}.
#'
#' See \code{\link{rfaControl}} for details on how these functions should be
#' defined.
#'
...
#' @author Max Kuhn
#' @seealso \code{\link{rfaControl}}, \code{\link{rfa}}
#' @keywords models
#' @examples
#'
...
#' the \code{rfa} object. The function \code{\link{rfaControl}} can be used to
...
#' produced (see the \code{method} argument of \code{\link{rfaControl}})
#'
#' @aliases xyplot.rfa stripplot.rfa densityplot.rfa histogram.rfa
#' @param x An object produced by \code{\link{rfa}}
...
#' @author Max Kuhn
#' @seealso \code{\link{rfa}}, \code{\link{rfaControl}},
#' \code{\link[lattice:histogram]{histogram}},
#' \code{\link[lattice:histogram]{densityplot}},
...
#' @export
predictors.rfa <- function(x, ...) x$optVariables

#' @export
varImp.rfa <- function(object, drop = FALSE, ...)
{
...
#' @export
predict.rfa <- function(object, newdata, ...)
{
  if(length(list(...)) > 0)
    warning("... are ignored for predict.rfa")

  if(inherits(object, "rfa.formula"))
  {
...
#' @method update rfa
#' @export
update.rfa <- function(object, x, y, size, ...) {
  size <- size[1]
